...
dir_len=$(find ${HOME}/RMS_data/CapturedFiles \
	       -maxdepth 1 -type d -name ${station} | wc -l)

printf "Number directories: %d\n" ${dir_len}

dir_array=($(find ${HOME}/RMS_data/CapturedFiles \
		  -maxdepth 1 -type d -name ${station} | sort -r))
